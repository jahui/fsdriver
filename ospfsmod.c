...
ospfs_dir_readdir(struct file *filp, void *dirent, filldir_t filldir)
{
  ...
  while (r == 0 && ok_so_far >= 0 && f_pos >= 2) {
    ...
    if((f_pos-2) >= (dir_oi->oi_size/OSPFS_DIRENTRY_SIZE)) //size of the entry / the size of directories
    {
      r = 1;
      break;
    }
    ...
    od = (ospfs_direntry_t*) ospfs_inode_data(dir_oi, (f_pos-2)*OSPFS_DIRENTRY_SIZE); // next entry
    entry_oi = ospfs_inode(od->od_ino); // inode for that next entry

    if(0 == od->od_ino ) // check for blank directory entries
      f_pos++; // skip it
    else
    {  
      switch (entry_oi->oi_ftype)
      { // depends on the type of file it is
        ...
        case OSPFS_FTYPE_DIR:
          ...
          break;
        case OSPFS_FTYPE_REG: // this is regular file type
...
ospfs_unlink(struct inode *dirino, struct dentry *dentry)
{
  ...
  // deleting symlinks
  if(oi->oi_ftype == OSPFS_FTYPE_SYMLINK) // don't decrement link count
    return 0;
...
{
  /* EXERCISE: Your code here */
  int block, fre, last_freemap_blk, retval, i;
  last_freemap_blk = (ospfs_super->os_firstinob) -1; // last free bitmap block
  for(block = OSPFS_FREEMAP_BLK; block < (last_freemap_blk+1); block++)
  {
    for(i = 0; i < OSPFS_BLKBITSIZE; i++) // for each bit in the block bitmap
    {
      // this first block's 0, 1, 2 bits shouldn't be tested
      // because they are the boot, super, bitmap respectively
      // but they should never be free anyway
      // so it's okay to test them
      fre = bitvector_test(&ospfs_data[block*OSPFS_BLKSIZE], i);
      if(fre) // free block!
      {
        bitvector_clear(&ospfs_data[block*OSPFS_BLKSIZE], i); // set to 0
        retval = block - OSPFS_FREEMAP_BLK; // the n-th block of freemap
        retval = retval*OSPFS_BLKBITSIZE; // the number of bits in n-1 blocks
        retval = retval + i; // the i-th bit in the n-th block 
        return retval;
      }
    }
  }
...
{
  /* EXERCISE: Your code here */
  int bitno, block, n_freemap_blk;
  int found = 0;

  n_freemap_blk = (ospfs_super->os_firstinob) - OSPFS_FREEMAP_BLK; // number of freemap blocks
  bitno = 0; // start at the 0th bit in the freemap (block 0)
  block = 0; // start from first freemap block

  while((block < n_freemap_blk) && !found)
  {
    if(blockno <= ((block + 1) * OSPFS_BLKBITSIZE)) // blockno is in the current bitmap block
    {
      bitno = blockno - (block * OSPFS_BLKBITSIZE);
      bitvector_set(&ospfs_data[(OSPFS_FREEMAP_BLK + block) * OSPFS_BLKSIZE], bitno);
      found = 1;
    }
    else
      block++;
...
{
  // Your code here.
  if(b >= OSPFS_NDIRECT + OSPFS_NINDIRECT) // block number is more than the number of blocks held in direct and indirect
    return 0;
  return -1;
}
...
indir_index(uint32_t b)
{
  ...
  if(b < OSPFS_NDIRECT) // direct block
    ...
    return ((b - (OSPFS_NDIRECT + OSPFS_NINDIRECT))/OSPFS_NINDIRECT); // integer division has no remainders
}
...
direct_index(uint32_t b)
{
  ...
  else // it is in the double indirect block
...
add_block(ospfs_inode_t *oi)
{
  ...
  uint32_t n = ospfs_size2nblocks(oi->oi_size); // the (n+1)th block will have a block index of n

  // keep track of allocations to free in case of -ENOSPC
  uint32_t allocated[3] = { 0, 0, 0};

  if(0 == indir2_index(n)) // needs a doubly indirect block
  {
    char* block; // block pointer
    uint32_t* ind_blk; // indirect block pointer
    uint32_t* dbl_blk; // dbl blk pointer

    if(!(oi->oi_indirect2 > 2)) // one has not already been made
    {
      allocated[0] = allocate_block();
      if (!allocated[0]) // if it did not allocate
        return -ENOSPC;

      block = (char*)ospfs_block(allocated[0]); // clear the newly allocated block byte by byte
      int i;
      for(i = 0; i < OSPFS_BLKSIZE; i++)
        block[i] = 0;

      oi->oi_indirect2 = allocated[0]; // set it
    }

    dbl_blk = (uint32_t*)ospfs_block(oi->oi_indirect2); // get a pointer to the double indirect block

    if (indir_index(n) != indir_index(n-1)) // belongs on a diff indirect block in the double block
    {
      allocated[1] = allocate_block(); // allocate for the new indirect block
      if (!allocated[1]) // if allocation failed
      {
        if(allocated[0]) // free the double indirect one we just made
        {
          free_block(allocated[0]);
          oi->oi_indirect2 = 0; // unset the pointer
        }
        return -ENOSPC;
      }

      block = (char*)ospfs_block(allocated[1]); // clear the block byte by byte
      int i;
      for(i = 0; i < OSPFS_BLKSIZE; i++)
        block[i] = 0;

      dbl_blk[indir_index(n)] = allocated[1]; // set the newly allocated block
    }

    allocated[2] = allocate_block(); // allocate for the actual block
    if (!allocated[2])
    {
      if (allocated[0])
      {
        free_block(allocated[0]); // error clearing, does more need to be done?
        oi->oi_indirect2 = 0;
      }
      if (allocated[1])
      {
        free_block(allocated[1]);
        dbl_blk[indir_index(n)] = 0; // unset the block
      }
      return -ENOSPC;
    }

    block = (char*)ospfs_block(allocated[2]); // clear the block byte by byte
    int i;
    for(i = 0; i < OSPFS_BLKSIZE; i++)
      block[i] = 0;
        
    ind_blk = (uint32_t*)ospfs_block(dbl_blk[indir_index(n)]); // get the indirect block data
    ind_blk[direct_index(n)] = allocated[2];
  }
  else if(0 == indir_index(n)) // its an indirect block
  {
    char* block;
    uint32_t* ind_blk;

    if(!(oi->oi_indirect > 2)) // if no indirect block made yet
    {
      ...
      block = (char*)ospfs_block(allocated[0]); // clear the block byte by byte
      int i;
      for(i = 0; i < OSPFS_BLKSIZE; i++)
        block[i] = 0;

      oi->oi_indirect = allocated[0]; // set the newly allocated block
    }
    allocated[1] = allocate_block(); // allocate a block for the actual block
    if (!allocated[1])
    {
      if(allocated[0])
      {
    ...
    ind_blk = (uint32_t*)ospfs_block(oi->oi_indirect); // get the indirect block data
    ind_blk[direct_index(n)] = allocated[1];
  }
  else // its a direct block
  {
    char* block;
    ...
    oi->oi_direct[direct_index(n)] = allocated[0]; // set direct index
  }
...
{
  // current number of blocks in file
  uint32_t n = ospfs_size2nblocks(oi->oi_size); // last block is actually n-1 index


  if(0 == indir2_index(n-1)) // it is in the double indirect
  {
    uint32_t* dbl_blk;
    int32_t idindex;
    uint32_t* ind_blk;
    ...
    if(!oi->oi_indirect2) // not yet allocated
      return -EIO;
    dbl_blk = (uint32_t*) ospfs_block(oi->oi_indirect2); // get the double block data
  
    idindex = indir_index(n-1); // indirect index variable;
    
    ind_blk = (uint32_t*) ospfs_block(dbl_blk[idindex]); // get the index block data
    
    dindex = direct_index(n-1); // direct index variable

    block = (char*) ospfs_block(ind_blk[dindex]); // clear the actual block, byte by byte
    int i;
    for(i = 0; i < OSPFS_BLKSIZE; i++)
      block[i] = 0;
    
    if(indir_index(n-2) != idindex) // if they are not in the same indirect index,
      ind_blk[dindex] = 0; // set the block pointer to 0

    if(0 != indir2_index(n-2)) // if one less is not in the double indirect
      oi->oi_indirect2 = 0;
  }
  else if(0 == indir_index(n-1)) // it is in the indirect index
  {
    uint32_t* ind_blk;
    char* block;
    if(!oi->oi_indirect) // not yet allocated
      return -EIO;
    ind_blk = (uint32_t*) ospfs_block(oi->oi_indirect); // get the index block data

    block = (char*) ospfs_block(ind_blk[direct_index(n-1)]); // clear the actual block, byte by byte
    int i;
    for(i = 0; i < OSPFS_BLKSIZE; i++)
      block[i] = 0;

    if(0 != indir_index(n-2)) // n-1 not in the indirect index
      oi->oi_indirect = 0;
  }
  else // it is in the direct index
  {
    int32_t dindex;
    char*block;

    dindex = direct_index(n-1);
    block = (char*) ospfs_block(oi->oi_direct[dindex]); // clear the block
    int i;
    for(i = 0; i < OSPFS_BLKSIZE; i++)
      block[i] = 0;

    oi->oi_direct[dindex] = 0; // clear the direct index
  }
  oi->oi_size -= OSPFS_BLKSIZE; // update size
...
change_size(ospfs_inode_t *oi, uint32_t new_size)
{
  ...
  oi->oi_size = new_size; // set the size
...
ospfs_read(struct file *filp, char __user *buffer, size_t count, loff_t *f_pos)
{
  ...
  /* EXERCISE: Your code here */
  if((count + *f_pos) > oi->oi_size) // if position plus count will go past file size
    count = oi->oi_size - *f_pos; // make it so that positon plus count will equal file size
  ...
  while (amount < count && retval >= 0) {
    ...
    uint32_t blkpos = *f_pos % OSPFS_BLKSIZE; // position in the block the file is pointing to 

    if((count - amount) > (OSPFS_BLKSIZE - blkpos)) // whats left to transfer is at least the rest of the block
      n = OSPFS_BLKSIZE - blkpos; // n is the rest of the block (total block - amount already read)
    else
      n = count - amount; // just transfer the rest

    retval = copy_to_user(buffer, data + blkpos, n); // copy over the data

    if (retval < 0) // check for failure
      return -EFAULT;
...
ospfs_write(struct file *filp, const char __user *buffer, size_t count, loff_t *f_pos)
{
  ...
  if((filp->f_flags & O_APPEND) == O_APPEND) // if the O_APPEND bit was set;
    *f_pos = oi->oi_size;
  ...
  int sizeleft = *f_pos + count;
  if(sizeleft > oi->oi_size)
  {
    retval = change_size(oi, sizeleft);
    if(retval < 0) // if the size change failed
      goto done;
  }
  ...
  while (amount < count && retval >= 0) {
    ...
    uint32_t blkpos = *f_pos % OSPFS_BLKSIZE; // the data it has already writen

    if( (count - amount) > (OSPFS_BLKSIZE - blkpos)) // whats left is more than whats left in the block
      n = OSPFS_BLKSIZE - blkpos; // total size - how much read so far of the block
    else
      n = count - amount;

    retval = copy_from_user(data+blkpos, buffer, n);
    if (retval < 0) // check for failure
      return -EFAULT;
...
create_blank_direntry(ospfs_inode_t *dir_oi)
{
  ...
  int add = add_block(dir_oi); // add a block
  if(add < 0) // error
    return ERR_PTR(add);
  
  // otherwise, clear out all the directory entries
  int diren;
  for(diren = off; diren < (off + OSPFS_BLKSIZE); diren += OSPFS_DIRENTRY_SIZE)
  {
    od = ospfs_inode_data(dir_oi, diren);
    od->od_ino = 0;
  }
...
ospfs_link(struct dentry *src_dentry, struct inode *dir, struct dentry *dst_dentry) 
{
  ...
  // check for dst name too long
  if (dst_dentry->d_name.len > OSPFS_MAXNAMELEN)
    return -ENAMETOOLONG;

  // check for file named same as dst_dentry already exisits
  ospfs_direntry_t *exist_od = find_direntry(dir_oi, dst_dentry->d_name.name, dst_dentry->d_name.len);
  if(exist_od != NULL)
    return -EEXIST;

  // set dst inode equal to src inode
  uint32_t ino;
  ino = src_dentry->d_inode->i_ino;

  // increase link count of the inode
  ospfs_inode_t *oi = ospfs_inode(ino);
  oi->oi_nlink++; // increment link count

  // create the direntry
  ospfs_direntry_t *od = create_blank_direntry(dir_oi); // create a blank direntry
  if(IS_ERR(od)) // error
    return PTR_ERR(od);
  od->od_ino = ino; // set inode number
  strncpy(od->od_name, dst_dentry->d_name.name, dst_dentry->d_name.len); // set the file name
  od->od_name[dst_dentry->d_name.len] = '\0'; // null terminate the name
...
ospfs_create(struct inode *dir, struct dentry *dentry, int mode, struct nameidata *nd)
{
  ...
  // check for EEXIST
  ospfs_direntry_t *exist_od = find_direntry(dir_oi, dentry->d_name.name, dentry->d_name.len);
  if(exist_od != NULL)
    return -EEXIST;
  ...
  for( ino = 0; ino < ospfs_super->os_ninodes; ino++)
  {
    oi = ospfs_inode(ino);
    if(oi->oi_nlink == 0) // empty inode
    {
      found = 1; // found!
      break;
    }
  ...
  // set the "entry_ino" to the inode number
  entry_ino = ino;
  ...
  // initialize the inode entry
  oi->oi_size = 0; // initialize size to 0
  oi->oi_ftype = OSPFS_FTYPE_REG; // regular file
  oi->oi_nlink = 1; // not free
  oi->oi_mode = mode; // set the file permissions mode
  memset(oi->oi_direct, 0, OSPFS_NDIRECT); // initialize direct
  oi->oi_indirect = NULL; // initialize indirect
  oi->oi_indirect2 = NULL; // initialize indirect2

  // initialize the directory entry
  od->od_ino = ino; // set inode number
  strncpy(od->od_name, dentry->d_name.name, dentry->d_name.len); // set file name
  od->od_name[dentry->d_name.len] = '\0'; // null terminate the name
...
ospfs_symlink(struct inode *dir, struct dentry *dentry, const char *symname)
{
  ...
  for( ino = 0; ino < ospfs_super->os_ninodes; ino++ )
  {
    oi = (ospfs_symlink_inode_t *) ospfs_inode(ino);
    if(oi->oi_nlink == 0) // empty inode
    {
      found = 1; // found!
      break;
  ...
  // make empty inode a symlink inode
  //ospfs_symlink_inode_t *osymi = (ospfs_symlink_inode_t *) ospfs_inode(ino);

  // set the "entry_ino" to the inode number
  entry_ino = ino;

  // initialize symlink inode
  oi->oi_size = strlen(symname); // number of characters in oi_symlink
  oi->oi_ftype = OSPFS_FTYPE_SYMLINK; // symbolic link
  oi->oi_nlink = 1; // not free anymore
  strcpy(oi->oi_symlink, symname); // set the oi_symlink
  //oi->oi_symlink[strlen(symname)] = '\0'; // null terminate the oi_symlink
...
ospfs_follow_link(struct dentry *dentry, struct nameidata *nd)
{
  ...
  int consym = 0; // is it a conditional symlink?
  if('r' == oi->oi_symlink[0] &&
     'o' == oi->oi_symlink[1] &&
     'o' == oi->oi_symlink[2] &&
     't' == oi->oi_symlink[3] &&
     '?' == oi->oi_symlink[4])
  ...
  int i = 5;
  // create the destination files for conditional symlink
  if(consym)
  {
   ...
   // int i = 5;
    ...
    i++; // increment i because original i is ':'
    // symbolic link destinations
    if( 0 == current->uid ) // root
      nd_set_link(nd, (oi->oi_symlink + 5));
    else // not root
      nd_set_link(nd, (oi->oi_symlink + i));
    return (void *) 0;
  }
